        ...
        <div className="container-lg mx-auto max-md:w-11/12">
          ...
          >
            {data.map(({ title, location, skills, jobId, orgname, _id }) => (
              <div
                key={_id}
                className="grid  grid-rows-[auto_1fr_auto] hover:shadow-md border-2 striped-border dark:bg-gray-900/30 bg-gray-50 relative rounded-md overflow-hidden"
              >
                <div className="p-2">
                  {jobId && (
                    <p className="font-mono text-gray-400 dark:text-gray-200 text-right text-xs capitalize max-md:text-sm">
                      {jobId}
                    </p>
                  )}
                  {title && (
                    <p className="font-medium text-lg dark:text-white capitalize max-md:text-sm">
                      {title}
                    </p>
                  )}
                  {orgname && (
                    <p className="font-normal dark:text-gray-50 text-sm capitalize max-md:text-sm">
                      {orgname.orgname}
                    </p>
                  )}
                  {location && (
                    <span className="font-medium max-md:font-normal dark:text-gray-50  text-sm capitalize text-gray-500">
                      {location}
                    </span>
                  )}
                </div>
