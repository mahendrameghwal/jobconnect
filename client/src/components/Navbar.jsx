...
import authApi, { useSignOutMutation, useCurrentUserQuery } from '../../app/api/authApi';
...
function Navbar() {
  ...
  return (
    <Fragment>
      {/*navbar */}
      <nav className={"z-40 dark:bg-gray-900/80 dark:border-b dark:border-b-slate-700 bg-slate-200/90 backdrop-blur-md sticky top-0 shadow-md flex items-center py-0.5 justify-between px-4 flex-wrap max-md:p-1"}>
        {/*logo */}
        <Link to={"/"} className="flex items-centerflex-shrink-0 mr-6 lg:mr-72">
          <img src={logo} className="w-100 h-7 mr-2" alt="Logo" />
        </Link>
        ...
        <div className={`w-full my-2 px-2 block flex-grow lg:flex lg:items-center lg:w-auto ${isOpen ? "block" : "hidden"}`}>
